...
const StudentStatusContent = ({ studentAuthData }: StudentStatusContentProps) => {
    ...
    const aiComment = useMemo(() => {
        ...
        if (chartData.classesHomework && chartData.classesHomework.length > 0) {
            const totalHw = chartData.classesHomework.find((h: any) => h.className === '총 달성률') || chartData.classesHomework[0];
            hwAvg = totalHw?.avgScore || 0;
        }
        
        let present = 0, absent = 0, late = 0;
        if (chartData.classesAttendance && chartData.classesAttendance.length > 0) {
            const totalAtt = chartData.classesAttendance.find((h: any) => h.className === '총 출석현황') || chartData.classesAttendance[0];
        ...
        try {
            ...
            const [praiseRes, studentRes] = await Promise.all([
                ...
                http.get('/choiMath/student/getStudentList', {
                    params: {
                        name: studentAuthData.name,
                        phoneNumber: studentAuthData.phone
                    }
                })
            ]);
            ...
            const myData = allData.find((s: any) => s.studentId === studentAuthData.studentId);
            
            // 학생 상세 정보 찾기 (학교, 학년)
            const matchedStudent = studentRes.data?.find((s: any) => s.studentId === studentAuthData.studentId);
            if (matchedStudent) {
            ...
            if (myData) {
                ...
                // 총 칭찬 순위 계산
                const sortedUniqueCounts = Array.from(new Set(allData.map((s: any) => s.totalPraiseCnt || 0))).sort((a: any, b: any) => b - a);
                ...
                if (myData.classes && myData.classes.length > 0) {
                    myData.classes.forEach((myClass: any) => {
                        ...
                        const sortedClassUniqueCounts = Array.from(new Set(allStudentsClassPraiseCounts)).sort((a: any, b: any) => b - a);
    ...
    const prepareChartData = (data: any) => {
        ...
        const classesAttendance: { className: string; attData: any; attOptions: any; total: number; percentage: number }[] = [];
        ...
        if (data.classes && data.classes.length > 0) {
            data.classes.forEach((c: any) => {
                ...
                c.attendance?.forEach((a: any) => {
                    if (a.status === 'class_present') { presentCount++; classPresent++; }
                    else if (a.status === 'class_absent') { absentCount++; classAbsent++; }
                    else if (a.status === 'late') { lateCount++; classLate++; }
                ...
                if (c.className && cTotalAtt > 0) {
                    ...
                    classesAttendance.push({ className: c.className, attData: cAttData, attOptions: cAttOptions, total: cTotalAtt, percentage });
                }
        ...
        if (totalAttendance > 0) {
            ...
            classesAttendance.push({ className: '총 출석현황', attData: totalAttData, attOptions: totalAttOptions, total: totalAttendance, percentage: totalPercentage });
        }
    ...
    const getAttendanceSeverity = (status: string) => {
        switch (status) {
            case 'class_present': return 'success';
            case 'class_absent': return 'danger';
        ...
        return (
            <div className="p-3">
                <DataTable value={filterAttendance} emptyMessage="칭찬 내역이 없습니다.">
                    <Column field="date" header="날짜" sortable body={(rowData: any) => dayjs(rowData?.date).format('YYYY-MM-DD')} />
                    <Column field="status" header="출석상태" body={(rowData: any) => (
                        <Tag
                            value={rowData.status === 'class_present' ? '출석' : rowData.status === 'class_absent' ? '결석' : '지각'}
                            severity={getAttendanceSeverity(rowData.status || '') as any}
                        />
                    )} />
                    <Column field="homework" header="숙제" body={(rowData: any) => `${rowData?.homework || 0}%`} />
                    <Column field="praise" header="칭찬여부" body={(rowData: any) =>
                        rowData.praise ? <i className="pi pi-face-smile text-green-500 text-2xl" /> : <i className="pi pi-minus text-400" />
                    } />
                </DataTable>
            </div>
        );
               ...
               {/* AI 학생 한줄평 영역 */}
                <div className="grid mt-2 mb-4">
                    <div className="col-12">
                        <div className="border-round-2xl shadow-1 p-4 flex flex-column align-items-center justify-content-center relative overflow-hidden" style={{ minHeight: '140px', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
                            ...
                            {aiAnalyzing ? (
                                (() => {
                                    let dynamicMessage = "";
                                    if (aiProgress < 20) dynamicMessage = "인공지능 슈퍼컴퓨터 전원을 켜기 위해 예열하는 중...사실없음... ⚡";
                                    else if (aiProgress < 40) dynamicMessage = "데이터베이스를 보안망을 피해 은밀하게 뒤적거리는 중... 🕵️";
                                    else if (aiProgress < 60) dynamicMessage = "방대한 숙제와 지각 기록들 사이에서 긍정적인 면을 찾아내는 중... 🔍";
                                    else if (aiProgress < 85) dynamicMessage = "잔소리를 할까 칭찬을 할까 선생님의 마음으로 깊이 고민하는 중... 🤔";
                                    else if (aiProgress < 100) dynamicMessage = "코멘트를 가장 예쁜 리본으로 정성스레 포장하는 중... 🎁";
                                    ...
                                    return (
                                        <div className="w-full md:w-8 z-1 flex flex-column align-items-center">
                                            ...
                                            <ProgressBar value={aiProgress} className="w-full border-round-3xl shadow-1" style={{ height: '18px', backgroundColor: 'rgba(255,255,255,0.2)' }} color="#facc15" displayValueTemplate={(val) => <span className="text-xs text-800 font-bold">{val}%</span>} />
                                        </div>
                                    );
                                })()
                            ) : (
                                <div className="flex align-items-center gap-3 z-1 w-full justify-content-center">
                                    ...
                                    <span className="text-lg md:text-2xl font-bold text-white line-height-3 text-center" style={{ textShadow: '1px 1px 3px rgba(0,0,0,0.3)' }}>
            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex align-items-center justify-content-between mb-5 bg-white p-4 border-round-2xl shadow-1">
                    <div className="flex align-items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl md:text-3xl font-bold m-0 text-900">{studentAuthData.name} 학생</h1>
                            <p className="text-500 m-0 mt-1">칭찬 현황을 확인해보세요!</p>
                        </div>
                    </div>
                    <div className="flex flex-column align-items-end gap-2">
                        <div className="flex align-items-center gap-2">
                            {(studentInfo.grade || studentAuthData.grade) && (
                                (() => {
                                    const gradeVal = String(studentInfo.grade || studentAuthData.grade);
                            ...
                            {(studentInfo.school || studentAuthData.school) && (
                                <Tag 
                                    value={studentInfo.school || studentAuthData.school} 
                                    severity="info" 
                ...
                <div className="grid gap-4 mb-4" style={{ margin: 0 }}>
                    {stats.classes?.map((c: any) => {
                        ...
                        return (
                            <div key={c.classId || c.className} className="col-12 sm:col-6 md:col lg:col flex-1 p-0">
                                <div className="bg-white p-4 border-round-2xl shadow-1 h-full flex flex-column justify-content-center align-items-center relative overflow-hidden" style={{ minHeight: '160px' }}>
                                    <div className="absolute top-0 right-0 p-3" style={{ opacity: 0.1 }}>
                                    ...
                                    {rank > 0 ? (
                                        <Tag 
                                            ...
                                            className={`px-3 py-1 text-sm border-round-xl ${rank <= 3 ? 'bg-yellow-500 text-white' : ''}`}
                                        />
                                    ) : (
                    ...
                    <div className="col-12 sm:col-6 md:col lg:col flex-1 p-0">
                        <div className="bg-white p-4 border-round-2xl shadow-1 h-full flex flex-column justify-content-center align-items-center relative overflow-hidden border-2 border-yellow-400" style={{ minHeight: '160px' }}>
                            <div className="absolute top-0 right-0 p-3" style={{ opacity: 0.1 }}>
                            ...
                            <Tag 
                                ...
                                className={`px-3 py-1 text-sm border-round-xl ${globalRank <= 3 ? 'bg-yellow-500 text-white' : ''}`}
                            />
                        </div>
                    </div>
                </div>
                                        ...
                                        return (
                                            <div key={idx} className="col-12 sm:col-6 md:col-4 mb-4">
                                                <div className="text-center flex flex-column align-items-center p-2">
                                                    <span className={`font-bold mb-3 text-overflow-ellipsis overflow-hidden white-space-nowrap w-full ${isTotal ? 'text-teal-600 text-lg' : 'text-700 text-base'}`} title={ca.className}>
                                                        {ca.className}
                                                    </span>
                                                    <div className="relative" style={{ width: '130px', height: '130px' }}>
                                                        <Chart type="doughnut" data={ca.attData} options={ca.attOptions} className="w-full h-full" />
                                                        <div className="absolute top-50 left-50" style={{ transform: 'translate(-50%, -50%)' }}>
                                                            <span className={`font-bold text-xl ${isTotal ? 'text-teal-600' : 'text-green-500'}`}>
                                                                {ca.percentage}%
                                                            </span>
                                                        </div>
                                                    </div>
                                ...
                                <div className="grid justify-content-center">
                                    {chartData.classesHomework.map((hc: any, idx: number) => {
                                        ...
                                        return (
                                            <div key={idx} className="col-12 sm:col-6 md:col-4 mb-4">
                                                <div className="text-center flex flex-column align-items-center p-2">
                                                    <span className={`font-bold mb-3 text-overflow-ellipsis overflow-hidden white-space-nowrap w-full ${isTotal ? 'text-indigo-600 text-lg' : 'text-700 text-base'}`} title={hc.className}>
                                                        {hc.className}
                                                    </span>
                                                    <div className="relative" style={{ width: '130px', height: '130px' }}>
                                                        <Chart type="doughnut" data={hc.hwData} options={hc.hwOptions} className="w-full h-full" />
                                                        <div className="absolute top-50 left-50" style={{ transform: 'translate(-50%, -50%)' }}>
                                                            <span className={`font-bold text-xl ${isTotal ? 'text-indigo-600' : 'text-blue-500'}`}>
                                                                {hc.avgScore}%
                                                            </span>
                                                        </div>
                                                    </div>
                ...
                <div className="grid mt-2">
                    <div className="col-12">
                        <Card title="클래스별 칭찬 내역 상세" className="border-none shadow-1 border-round-2xl overflow-hidden">
                            <DataTable 
                                value={stats.classes} 
                            ...
                            >
                                ...
                                <Column header="칭찬 횟수" body={(rowData) => {
                                ...
                                <Column header="순위" body={(rowData) => {
                                    const rank = classRanks[rowData.classId] || 0;
                                    if (rank === 0) return <span>-</span>;
                                    let textColor = 'text-600';
                                    let trophyColor = '';
                                    if (rank === 1) { textColor = 'text-yellow-600'; trophyColor = '#FFD700'; }
                                    else if (rank === 2) { textColor = 'text-gray-500'; trophyColor = '#C0C0C0'; }
                                    else if (rank === 3) { textColor = 'text-orange-600'; trophyColor = '#CD7F32'; }
